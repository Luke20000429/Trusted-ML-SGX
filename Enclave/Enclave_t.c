...
static sgx_status_t SGX_CDECL sgx_unseal(void* pms)
{
	...
	_in_retval = unseal(_in_sealed_data, _tmp_sealed_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ecall_classify(void* pms)
{
	...
	char* _tmp_weights = __in_ms.ms_weights;


	ecall_classify(_tmp_sections, _tmp_labels, _tmp_im, _tmp_weights, __in_ms.ms_image_num);
